...
SIMTRACE_CSV_DATA_HEADER = ['episode', 'steps', 'X', 'Y', 'Z', 'pitch', 'yaw', 'roll', 'velocity_rotor_1', 'velocity_rotor_2', 'velocity_rotor_3', 'velocity_rotor_4', 'action', 'reward', 'done', 'tstamp']
...
if node_type == SIMULATION_WORKER:
    ...
    class DeepRotorSimTraceData():
        __instance = None
        @staticmethod
        def getInstance():
            """ Static access method. """
            if DeepRotorSimTraceData.__instance == None:
               logger.info("simtrace_data: Instantiate singleton")
               DeepRotorSimTraceData("None", "None")
            return DeepRotorSimTraceData.__instance
        def __init__(self, s3_bucket, s3_key):
            """ Virtually private constructor. """
            if DeepRotorSimTraceData.__instance != None:
                logger.info ("simtrace-data: DeepRotorSimTraceData - already instantiated, should not happen")
            else:
                logger.info("simtrace_data init")
                DeepRotorSimTraceData.__instance = self
